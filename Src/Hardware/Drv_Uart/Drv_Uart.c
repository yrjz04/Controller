 ...
 * @LastEditTime: 2026-01-11 16:55:39
...
UART_RxTypeDef Uart2 = {&huart2, NULL, {0}, 0, 0, UART2_NODMA_ENABLE};  // 为UART2初始化结构体
#endif
#if UART2_DMA_ENABLE
...
void Uart_Init(UART_RxTypeDef *UartRx)
{
    if(UartRx->UseDMA)
    {
        HAL_UARTEx_ReceiveToIdle_DMA(UartRx->huart, UartRx->RxBuffer, RX_BUFFER_SIZE);
        __HAL_DMA_DISABLE_IT(UartRx->hdma_rx, DMA_IT_HT);
    }
    else
    {
        HAL_UARTEx_ReceiveToIdle_IT(UartRx->huart, UartRx->RxBuffer, RX_BUFFER_SIZE);
    }
}
